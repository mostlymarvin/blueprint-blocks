...
const SOCIALTEMPLATE = [["blueprint-blocks/social-link", {}, []]];
...
class myAuthorEdit extends Component {
		...
		theAuthors.fetch().then(authors => {
			if (authors && 0 !== this.state.selectedAuthor) {
				...
				const author = authors.find(item => {
					return item.id == this.state.selectedAuthor;
				});
			...
			<InspectorControls>
				...
				>
					...
					{this.props.attributes.align === "full" && (
						<PanelRow className="width-control">
							<RangeControl
		...
		return this.state.siteLinks ? (
			<div className="blueprint-profile-links site-links">
				{this.state.siteLinks.map((item, key) => {
					...
					return (
						<span
							className={item.network + " icon-" + item.network}
							style={socialLinkBg}
						>
							<span className="editor-icon" style={socialIconStyle}></span>
						</span>
					);
				})}
			</div>
	...
	onChangeSelectAuthor(value) {
		// Find the author
		const author = this.state.authors.find(item => {
	...
	getBlockSettings(options, message) {
		return this.props.attributes.mbtActive ? (
			...
			<div className="blueprint-block-settings">
				<div className="blueprint-block-message">
					...
					<span>
						...
						Please activate{" "}
						<a
							href="https://wordpress.org/plugins/mybooktable/"
							target="_blank"
						>
							MyBookTable
						</a>{" "}
						and create Authors first.
					</span>
				</div>
			</div>
		);
	...
	render() {
		...
		let placeholder =
			"Cras justo odio, dapibus ac facilisis in, egestas eget quam. Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh, ut fermentum massa justo sit amet risus. Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum.";
		let spanStyle = {};
		...
		if (this.state.authors.length > 0) {
			...
			message =
				"No authors found - please install MyBookTable or create author first.";
		}

		// Checking if we have anything in the object
		if (
			this.state.author !== undefined &&
			this.state.author.hasOwnProperty("name")
		) {
			output = (
				<div className="author">
					<a href={this.state.author.link}>{this.state.author.name}</a>
				</div>
			);
		}
								...
								<div class="profile">
									...
									{this.props.attributes.showReadMore &&
										this.props.attributes.readMoreLink && (
											<div
												className={
													"bpb-more-link " + this.props.attributes.alignReadMore
												}
											>
												<a
...
registerBlockType("blueprint-blocks/author-profile", {
	...
	keywords: [__("Author"), __("Blueprint"), __("Profile")],
	supports: {
	...
	save: function(props) {
		...
		const socialShortcode =
			'[blueprint_social class="author-profile" direction="column" size="small"]';
		return (
			props.attributes.selectedAuthor && (
				<div
					...
					style={blockStyle}
				>
					<div className="inner-wrap" style={innerStyle}>
						<h2 className="profile-title section-title" style={h3Style}>
							<RichText.Content
								tagName="span"
								className="author-name"
								...
								<div className="profile">
									...
									{props.attributes.showReadMore &&
										props.attributes.readMoreLink && (
											<div
												className={
													"bpb-more-link " + props.attributes.alignReadMore
												}
											>
												<a
													className={buttonClass + " readMoreLink"}
